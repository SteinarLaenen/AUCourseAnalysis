...
def sentiwordnetanalysis(doc, obj):
	"""
	doc is lemmatized, post_tagged tokenized list as input in format 
        ["word-POS"] as above. returns the polarity score of this doc.
	obj is boundary under which the objectivity score has to be in order for 
        a word to count towards the sentiment analysis Seems to be optimal in
        0.4-0.6 range
        """
	total_polarity = 0
	positive = 0
	...
	for w in doc:
		if w.split("-")[-1] in ["ADJ", "ADV", "NOUN", "VERB"]:
			if (swn.senti_synsets(
                                        w.split("-")[0],
                                        pos = un2wn_mapping[w.split("-")[-1]])
                        != []):
...
def run(*args):
        ...
        for obj in np.arange(0, 1.1, 0.1):
                ...
                number_neutral = 0
                print "loading:", obj
                for comment in Comment.objects.filter(review=True):
                        if (comment.polarity_gs == -1 or
                            comment.polarity_gs == 1):
                                comment.polarity_predicted = sentiwordnetanalysis(
                                        preprocesscomment(comment.text), obj)
                                # Keep track of how many pos/neg have been counted
                                if (number_negative != 50 and
                                    comment.polarity_gs == -1):
                                        if (comment.polarity_predicted ==
                                            comment.polarity_gs):
                                                number_correct += 1

                                        number_negative += 1
                                        test_set_size += 1
                                if (number_positive != 50 and
                                    comment.polarity_gs == 1):
                                        if (comment.polarity_predicted ==
                                            comment.polarity_gs):
                                                number_correct += 1
        ...
        for comment in Comment.objects.all():
                polarity = sentiwordnetanalysis(preprocesscomment(comment.text),
                                                0.4)
                comment.polarity_predicted = polarity
                comment.save()
